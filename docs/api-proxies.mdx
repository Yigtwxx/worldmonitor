...
- Apply per-IP rate limits via `api/_rate-limit.js` or `server/_shared/rate-limit.ts` (600 req/min/IP default), with stricter per-route budgets declared in `ENDPOINT_RATE_POLICIES` — see [Rate Limits](/usage-rate-limits).
