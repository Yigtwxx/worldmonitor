...
Routes that fetch a third-party host on our behalf carry their own per-IP budget, so a scripted caller cannot get our egress IPs blocked by a provider we do not control.
